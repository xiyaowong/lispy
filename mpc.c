...
static mpc_input_t*
mpc_input_new_string(const char* filename, const char* string)
{
...
static mpc_input_t*
mpc_input_new_nstring(const char* filename, const char* string, size_t length)
{
...
static mpc_input_t*
mpc_input_new_pipe(const char* filename, FILE* pipe)
{
...
static mpc_input_t*
mpc_input_new_file(const char* filename, FILE* file)
{
...
static void
mpc_input_delete(mpc_input_t* i)
{
...
static int
mpc_mem_ptr(mpc_input_t* i, void* p)
{
...
static void*
mpc_malloc(mpc_input_t* i, size_t n)
{
  size_t j;
  char* p;
...
static void*
mpc_calloc(mpc_input_t* i, size_t n, size_t m)
{
...
static void*
mpc_realloc(mpc_input_t* i, void* p, size_t n)
{
...
static void*
mpc_export(mpc_input_t* i, void* p)
{
  char* q = NULL;
  if (!mpc_mem_ptr(i, p)) {
    return p;
...
static void
mpc_input_backtrack_disable(mpc_input_t* i)
{
  i->backtrack--;
}
static void
mpc_input_backtrack_enable(mpc_input_t* i)
{
  i->backtrack++;
}

static void
mpc_input_suppress_disable(mpc_input_t* i)
{
  i->suppress--;
}
static void
mpc_input_suppress_enable(mpc_input_t* i)
{
  i->suppress++;
}

static void
mpc_input_mark(mpc_input_t* i)
{
...
static void
mpc_input_unmark(mpc_input_t* i)
{
  int j;
...
static void
mpc_input_rewind(mpc_input_t* i)
{
...
static int
mpc_input_buffer_in_range(mpc_input_t* i)
{
  return i->state.pos < (long)(strlen(i->buffer) + i->marks[0].pos);
}

static char
mpc_input_buffer_get(mpc_input_t* i)
{
  return i->buffer[i->state.pos - i->marks[0].pos];
}

static char
mpc_input_getc(mpc_input_t* i)
{
...
static int
mpc_input_terminated(mpc_input_t* i)
{
  return mpc_input_peekc(i) == '\0';
}

static int
mpc_input_failure(mpc_input_t* i, char c)
{
...
static int
mpc_input_success(mpc_input_t* i, char c, char** o)
{
...
static int
mpc_input_any(mpc_input_t* i, char** o)
{
  char x;
...
static int
mpc_input_char(mpc_input_t* i, char c, char** o)
{
  char x;
  if (mpc_input_terminated(i)) {
    return 0;
...
static int
mpc_input_range(mpc_input_t* i, char c, char d, char** o)
{
  char x;
  if (mpc_input_terminated(i)) {
    return 0;
...
static int
mpc_input_oneof(mpc_input_t* i, const char* c, char** o)
{
  char x;
  if (mpc_input_terminated(i)) {
    return 0;
...
static int
mpc_input_noneof(mpc_input_t* i, const char* c, char** o)
{
  char x;
  if (mpc_input_terminated(i)) {
    return 0;
...
static int
mpc_input_satisfy(mpc_input_t* i, int (*cond)(char), char** o)
{
  char x;
  if (mpc_input_terminated(i)) {
    return 0;
...
static int
mpc_input_string(mpc_input_t* i, const char* c, char** o)
{
...
static int
mpc_input_anchor(mpc_input_t* i, int (*f)(char, char), char** o)
{
  *o = NULL;
...
static int
mpc_input_soi(mpc_input_t* i, char** o)
{
  *o = NULL;
  return i->last == '\0';
}
...
static int
mpc_input_eoi(mpc_input_t* i, char** o)
{
  *o = NULL;
  if (i->state.term) {
    return 0;
...
static mpc_state_t*
mpc_input_state_copy(mpc_input_t* i)
{
...
void
mpc_err_print(mpc_err_t* x)
{
  mpc_err_print_to(x, stdout);
}

void
mpc_err_print_to(mpc_err_t* x, FILE* f)
{
...
static void
mpc_err_string_cat(char* buffer, int* pos, int* max, char const* fmt, ...)
{
...
mpc_err_string(mpc_err_t* x)
{
  ...
  if (x->failure) {
    mpc_err_string_cat(
      buffer, &pos, &max, "%s: error: %s\n", x->filename, x->failure);
    return buffer;
  }

  mpc_err_string_cat(buffer,
                     &pos,
                     &max,
                     "%s:%li:%li: error: expected ",
                     x->filename,
                     x->state.row + 1,
                     x->state.col + 1);
...
static mpc_err_t*
mpc_err_new(mpc_input_t* i, const char* expected)
{
  mpc_err_t* x;
  if (i->suppress) {
    return NULL;
...
static mpc_err_t*
mpc_err_fail(mpc_input_t* i, const char* failure)
{
  mpc_err_t* x;
  if (i->suppress) {
    return NULL;
...
static mpc_err_t*
mpc_err_file(const char* filename, const char* failure)
{
  mpc_err_t* x;
...
static void
mpc_err_delete_internal(mpc_input_t* i, mpc_err_t* x)
{
  int j;
  if (x == NULL) {
    return;
...
static mpc_err_t*
mpc_err_export(mpc_input_t* i, mpc_err_t* x)
{
  int j;
  for (j = 0; j < x->expected_num; j++) {
...
static int
mpc_err_contains_expected(mpc_input_t* i, mpc_err_t* x, char* expected)
{
  int j;
  (void)i;
  for (j = 0; j < x->expected_num; j++) {
...
static void
mpc_err_add_expected(mpc_input_t* i, mpc_err_t* x, char* expected)
{
  (void)i;
  x->expected_num++;
...
static mpc_err_t*
mpc_err_or(mpc_input_t* i, mpc_err_t** x, int n)
{
...
static mpc_err_t*
mpc_err_repeat(mpc_input_t* i, mpc_err_t* x, const char* prefix)
{
...
static mpc_err_t*
mpc_err_many1(mpc_input_t* i, mpc_err_t* x)
{
  return mpc_err_repeat(i, x, "one or more of ");
}

static mpc_err_t*
mpc_err_count(mpc_input_t* i, mpc_err_t* x, int n)
{
  mpc_err_t* y;
  int digits = n / 10 + 1;
  char* prefix;
...
static mpc_err_t*
mpc_err_merge(mpc_input_t* i, mpc_err_t* x, mpc_err_t* y)
{
  mpc_err_t* errs[2];
  errs[0] = x;
  errs[1] = y;
...
static mpc_val_t*
mpcf_input_nth_free(mpc_input_t* i, int n, mpc_val_t** xs, int x)
{
  int j;
  for (j = 0; j < n; j++) {
    if (j != x) {
...
static mpc_val_t*
mpcf_input_fst_free(mpc_input_t* i, int n, mpc_val_t** xs)
{
  return mpcf_input_nth_free(i, n, xs, 0);
}
static mpc_val_t*
mpcf_input_snd_free(mpc_input_t* i, int n, mpc_val_t** xs)
{
  return mpcf_input_nth_free(i, n, xs, 1);
}
static mpc_val_t*
mpcf_input_trd_free(mpc_input_t* i, int n, mpc_val_t** xs)
{
  return mpcf_input_nth_free(i, n, xs, 2);
}

static mpc_val_t*
mpcf_input_strfold(mpc_input_t* i, int n, mpc_val_t** xs)
{
  int j;
  size_t l = 0;
  if (n == 0) {
...
static mpc_val_t*
mpcf_input_state_ast(mpc_input_t* i, int n, mpc_val_t** xs)
{
...
static mpc_val_t*
mpc_parse_fold(mpc_input_t* i, mpc_fold_t f, int n, mpc_val_t** xs)
{
  int j;
  if (f == mpcf_null) {
...
static mpc_val_t*
mpcf_input_free(mpc_input_t* i, mpc_val_t* x)
{
  mpc_free(i, x);
  return NULL;
}
...
static mpc_val_t*
mpcf_input_str_ast(mpc_input_t* i, mpc_val_t* c)
{
...
static mpc_val_t*
mpc_parse_apply(mpc_input_t* i, mpc_apply_t f, mpc_val_t* x)
{
  if (f == mpcf_free) {
...
static mpc_val_t*
mpc_parse_apply_to(mpc_input_t* i, mpc_apply_to_t f, mpc_val_t* x, mpc_val_t* d)
{
  return f(mpc_export(i, x), d);
}

static void
mpc_parse_dtor(mpc_input_t* i, mpc_dtor_t d, mpc_val_t* x)
{
  if (d == free) {
    mpc_free(i, x);
    return;
  ...
  switch (p->type) {
    ...
    case MPC_TYPE_RANGE:
      MPC_PRIMITIVE(mpc_input_range(
        i, p->data.range.x, p->data.range.y, (char**)&r->output));
    case MPC_TYPE_ONEOF:
    ...
    case MPC_TYPE_APPLY_TO:
      if (mpc_parse_run(i, p->data.apply_to.x, r, e, depth + 1)) {
        MPC_SUCCESS(mpc_parse_apply_to(
          i, p->data.apply_to.f, r->output, p->data.apply_to.d));
      } else {
        MPC_FAILURE(r->error);
      }
    ...
    case MPC_TYPE_MANY:
      ...
      while (mpc_parse_run(i, p->data.repeat.x, &results[j], e, depth + 1)) {
        ...
        if (j == MPC_PARSE_STACK_MIN) {
          ...
          memcpy(
            results, results_stk, sizeof(mpc_result_t) * MPC_PARSE_STACK_MIN);
        } else if (j >= results_slots) {
          results_slots = j + j / 2;
          results =
            mpc_realloc(i, results, sizeof(mpc_result_t) * results_slots);
        }
      }
...
int
mpc_parse_input(mpc_input_t* i, mpc_parser_t* p, mpc_result_t* r)
{
  int x;
...
int
mpc_nparse(const char* filename,
           const char* string,
           size_t length,
           mpc_parser_t* p,
           mpc_result_t* r)
...
int
mpc_parse_file(const char* filename,
               FILE* file,
               mpc_parser_t* p,
               mpc_result_t* r)
{
...
int
mpc_parse_pipe(const char* filename,
               FILE* pipe,
               mpc_parser_t* p,
               mpc_result_t* r)
{
...
int
mpc_parse_contents(const char* filename, mpc_parser_t* p, mpc_result_t* r)
{
...
static void
mpc_undefine_unretained(mpc_parser_t* p, int force);

static void
mpc_undefine_or(mpc_parser_t* p)
{
...
static void
mpc_undefine_and(mpc_parser_t* p)
{
...
mpc_parser_t*
mpc_define(mpc_parser_t* p, mpc_parser_t* a)
{
...
mpc_parser_t*
mpc_expect(mpc_parser_t* a, const char* expected)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_expectf(mpc_parser_t* a, const char* fmt, ...)
{
  va_list va;
  char* buffer;
...
mpc_parser_t*
mpc_apply(mpc_parser_t* a, mpc_apply_t f)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_apply_to(mpc_parser_t* a, mpc_apply_to_t f, void* x)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_check(mpc_parser_t* a, mpc_dtor_t da, mpc_check_t f, const char* e)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_checkf(mpc_parser_t* a, mpc_dtor_t da, mpc_check_t f, const char* fmt, ...)
{
  va_list va;
  char* buffer;
  mpc_parser_t* p;
...
mpc_parser_t*
mpc_check_withf(mpc_parser_t* a,
                mpc_dtor_t da,
                mpc_check_with_t f,
                void* x,
                const char* fmt,
...
mpc_parser_t*
mpc_not_lift(mpc_parser_t* a, mpc_dtor_t da, mpc_ctor_t lf)
{
  mpc_parser_t* p = mpc_undefined();
  p->type = MPC_TYPE_NOT;
  p->data.not .x = a;
...
mpc_parser_t*
mpc_not(mpc_parser_t* a, mpc_dtor_t da)
{
  return mpc_not_lift(a, da, mpcf_ctor_null);
}

mpc_parser_t*
mpc_maybe_lift(mpc_parser_t* a, mpc_ctor_t lf)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_maybe(mpc_parser_t* a)
{
  return mpc_maybe_lift(a, mpcf_ctor_null);
}

mpc_parser_t*
mpc_many(mpc_fold_t f, mpc_parser_t* a)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_many1(mpc_fold_t f, mpc_parser_t* a)
{
  mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t*
mpc_count(int n, mpc_fold_t f, mpc_parser_t* a, mpc_dtor_t da)
{
  mpc_parser_t* p = mpc_undefined();
...
static int
mpc_boundary_anchor(char prev, char next)
{
...
static int
mpc_boundary_newline_anchor(char prev, char next)
{
  (void)next;
  return prev == '\n';
}
...
mpc_real(void)
{
  ...
  p1 = mpc_digits();
  p2 = mpc_maybe_lift(
    mpc_and(2, mpcf_strfold, mpc_char('.'), mpc_digits(), free), mpcf_ctor_str);
  p30 = mpc_oneof("eE");
...
mpc_parser_t*
mpc_startwith(mpc_parser_t* a)
{
  return mpc_and(2, mpcf_snd, mpc_soi(), a, mpcf_dtor_null);
}
mpc_parser_t*
mpc_endwith(mpc_parser_t* a, mpc_dtor_t da)
{
...
{
  return mpc_and(
    3, mpcf_snd, mpc_blank(), a, mpc_blank(), mpcf_dtor_null, mpcf_dtor_null);
}
mpc_parser_t*
...
mpc_parser_t*
mpc_total(mpc_parser_t* a, mpc_dtor_t da)
{
  return mpc_whole(mpc_strip(a), da);
}

mpc_parser_t*
mpc_between(mpc_parser_t* a, mpc_dtor_t ad, const char* o, const char* c)
{
...
mpc_parser_t*
mpc_brackets(mpc_parser_t* a, mpc_dtor_t ad)
{
  return mpc_between(a, ad, "{", "}");
}
mpc_parser_t*
mpc_squares(mpc_parser_t* a, mpc_dtor_t ad)
{
  return mpc_between(a, ad, "[", "]");
}

mpc_parser_t*
mpc_tok_between(mpc_parser_t* a, mpc_dtor_t ad, const char* o, const char* c)
{
  return mpc_and(
    3, mpcf_snd_free, mpc_sym(o), mpc_tok(a), mpc_sym(c), free, ad);
}
...
static mpc_val_t*
mpcf_re_and(int n, mpc_val_t** xs)
{
  int i;
...
static mpc_val_t*
mpcf_re_repeat(int n, mpc_val_t** xs)
{
  int num;
  (void)n;
  if (xs[1] == NULL) {
...
mpc_re_escape_char(char c)
{
  switch (c) {
    ...
    case 'b':
      return mpc_and(
        2, mpcf_snd, mpc_boundary(), mpc_lift(mpcf_ctor_str), free);
    case 'B':
...
static mpc_val_t*
mpcf_re_escape(mpc_val_t* x, void* data)
{
...
static const char*
mpc_re_range_escape_char(char c)
{
  switch (c) {
    case '-':
      return "-";
...
mpc_parser_t*
mpc_re(const char* re)
{
  return mpc_re_mode(re, MPC_RE_DEFAULT);
}

mpc_parser_t*
mpc_re_mode(const char* re, int mode)
{
  ...
  mpc_define(
    Range,
    mpc_apply(mpc_many(mpcf_strfold, mpc_or(2, mpc_escape(), mpc_noneof("]"))),
              mpcf_re_range));
...
static mpc_val_t*
mpcf_escape_new(mpc_val_t* x, const char* input, const char** output)
{
...
static mpc_val_t*
mpcf_unescape_new(mpc_val_t* x, const char* input, const char** output)
{
...
mpc_val_t*
mpcf_unescape_regex(mpc_val_t* x)
{
  mpc_val_t* y =
...
mpc_val_t*
mpcf_escape_string_raw(mpc_val_t* x)
{
  mpc_val_t* y =
...
mpc_val_t*
mpcf_unescape_string_raw(mpc_val_t* x)
{
  mpc_val_t* y =
    mpcf_unescape_new(x, mpc_escape_input_raw_cstr, mpc_escape_output_raw_cstr);
  free(x);
  return y;
}

mpc_val_t*
mpcf_escape_char_raw(mpc_val_t* x)
{
  mpc_val_t* y =
    mpcf_escape_new(x, mpc_escape_input_raw_cchar, mpc_escape_output_raw_cchar);
  free(x);
  return y;
}

mpc_val_t*
mpcf_unescape_char_raw(mpc_val_t* x)
{
  mpc_val_t* y = mpcf_unescape_new(
    x, mpc_escape_input_raw_cchar, mpc_escape_output_raw_cchar);
  free(x);
  return y;
}

mpc_val_t*
mpcf_null(int n, mpc_val_t** xs)
{
  (void)n;
  (void)xs;
  return NULL;
...
mpc_val_t*
mpcf_fst(int n, mpc_val_t** xs)
{
  (void)n;
  return xs[0];
}
mpc_val_t*
mpcf_snd(int n, mpc_val_t** xs)
{
  (void)n;
  return xs[1];
}
mpc_val_t*
mpcf_trd(int n, mpc_val_t** xs)
{
  (void)n;
  return xs[2];
}
...
static mpc_val_t*
mpcf_nth_free(int n, mpc_val_t** xs, int x)
{
  int i;
  for (i = 0; i < n; i++) {
    if (i != x) {
...
mpc_val_t*
mpcf_fst_free(int n, mpc_val_t** xs)
{
  return mpcf_nth_free(n, xs, 0);
}
mpc_val_t*
mpcf_snd_free(int n, mpc_val_t** xs)
{
  return mpcf_nth_free(n, xs, 1);
}
mpc_val_t*
mpcf_trd_free(int n, mpc_val_t** xs)
{
  return mpcf_nth_free(n, xs, 2);
}
mpc_val_t*
mpcf_all_free(int n, mpc_val_t** xs)
{
  int i;
  for (i = 0; i < n; i++) {
    free(xs[i]);
...
mpc_val_t*
mpcf_strfold(int n, mpc_val_t** xs)
{
  int i;
  size_t l = 0;
...
static void
mpc_print_unretained(mpc_parser_t* p, int force)
{
  ...
  if (p->type == MPC_TYPE_ONEOF) {
    s = mpcf_escape_new(
      p->data.string.x, mpc_escape_input_c, mpc_escape_output_c);
    printf("[%s]", s);
    free(s);
  }
...
static void
mpc_ast_delete_no_children(mpc_ast_t* a)
{
  free(a->children);
  free(a->tag);
  free(a->contents);
...
mpc_ast_t*
mpc_ast_new(const char* tag, const char* contents)
{
...
mpc_ast_t*
mpc_ast_build(int n, const char* tag, ...)
{
...
int
mpc_ast_eq(mpc_ast_t* a, mpc_ast_t* b)
{
...
mpc_ast_t*
mpc_ast_add_child(mpc_ast_t* r, mpc_ast_t* a)
{
  r->children_num++;
...
mpc_ast_t*
mpc_ast_add_tag(mpc_ast_t* a, const char* t)
{
  if (a == NULL) {
    return a;
  }
...
mpc_ast_t*
mpc_ast_add_root_tag(mpc_ast_t* a, const char* t)
{
  if (a == NULL) {
    return a;
  }
...
mpc_ast_t*
mpc_ast_tag(mpc_ast_t* a, const char* t)
{
...
mpc_ast_t*
mpc_ast_state(mpc_ast_t* a, mpc_state_t s)
{
  if (a == NULL) {
    return a;
  }
...
static void
mpc_ast_print_depth(mpc_ast_t* a, int d, FILE* fp)
{
...
void
mpc_ast_print(mpc_ast_t* a)
{
  mpc_ast_print_depth(a, 0, stdout);
}

void
mpc_ast_print_to(mpc_ast_t* a, FILE* fp)
{
...
int
mpc_ast_get_index_lb(mpc_ast_t* ast, const char* tag, int lb)
{
  int i;
...
mpc_ast_t*
mpc_ast_get_child(mpc_ast_t* ast, const char* tag)
{
  return mpc_ast_get_child_lb(ast, tag, 0);
}

mpc_ast_t*
mpc_ast_get_child_lb(mpc_ast_t* ast, const char* tag, int lb)
{
  int i;
...
mpc_ast_trav_t*
mpc_ast_traverse_start(mpc_ast_t* ast, mpc_ast_trav_order_t order)
{
...
mpc_ast_t*
mpc_ast_traverse_next(mpc_ast_trav_t** trav)
{
...
void
mpc_ast_traverse_free(mpc_ast_trav_t** trav)
{
...
mpc_val_t*
mpcf_state_ast(int n, mpc_val_t** xs)
{
  mpc_state_t* s = ((mpc_state_t**)xs)[0];
  mpc_ast_t* a = ((mpc_ast_t**)xs)[1];
  (void)n;
...
static mpc_val_t*
mpcaf_grammar_or(int n, mpc_val_t** xs)
{
  (void)n;
  if (xs[1] == NULL) {
    return xs[0];
...
static mpc_val_t*
mpcaf_grammar_and(int n, mpc_val_t** xs)
{
  int i;
...
static mpc_val_t*
mpcaf_grammar_repeat(int n, mpc_val_t** xs)
{
  int num;
  (void)n;
  if (xs[1] == NULL) {
...
static mpc_val_t*
mpcaf_grammar_string(mpc_val_t* x, void* s)
{
...
static mpc_val_t*
mpcaf_grammar_char(mpc_val_t* x, void* s)
{
  mpca_grammar_st_t* st = s;
  char* y = mpcf_unescape(x);
  mpc_parser_t* p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE)
...
static mpc_val_t*
mpcaf_fold_regex(int n, mpc_val_t** xs)
{
  char* y = xs[0];
  char* m = xs[1];
...
static mpc_parser_t*
mpca_grammar_find_parser(char* x, mpca_grammar_st_t* st)
{
...
static mpc_val_t*
mpcaf_grammar_id(mpc_val_t* x, void* s)
{
...
mpc_parser_t*
mpca_grammar_st(const char* grammar, mpca_grammar_st_t* st)
{
  ...
  mpc_define(
    ...
    mpc_or(
      ...
      mpc_tok(mpc_and(3,
                      ...
                      free,
                      free)),
      mpc_apply_to(mpc_tok_braces(mpc_or(2, mpc_digits(), mpc_ident()), free),
                   mpcaf_grammar_id,
                   st),
...
mpc_parser_t*
mpca_grammar(int flags, const char* grammar, ...)
{
...
static mpc_val_t*
mpca_stmt_afold(int n, mpc_val_t** xs)
{
...
static mpc_val_t*
mpca_stmt_fold(int n, mpc_val_t** xs)
{
...
static void
mpca_stmt_list_delete(mpc_val_t* x)
{
...
static mpc_val_t*
mpca_stmt_list_apply_to(mpc_val_t* x, void* s)
{
...
static mpc_err_t*
mpca_lang_st(mpc_input_t* i, mpca_grammar_st_t* st)
{
  ...
  mpc_define(Stmt,
             mpc_and(5,
                     mpca_stmt_afold,
                     mpc_tok(mpc_ident()),
                     mpc_maybe(mpc_tok(mpc_string_lit())),
                     mpc_sym(":"),
                     Grammar,
                     mpc_sym(";"),
                     free,
...
mpc_err_t*
mpca_lang_file(int flags, FILE* f, ...)
{
  mpca_grammar_st_t st;
  mpc_input_t* i;
  mpc_err_t* err;
...
mpc_err_t*
mpca_lang_pipe(int flags, FILE* p, ...)
{
  mpca_grammar_st_t st;
  mpc_input_t* i;
  mpc_err_t* err;
...
mpc_err_t*
mpca_lang(int flags, const char* language, ...)
{
...
mpc_err_t*
mpca_lang_contents(int flags, const char* filename, ...)
{
...
static int
mpc_nodecount_unretained(mpc_parser_t* p, int force)
{
...
static void
mpc_optimise_unretained(mpc_parser_t* p, int force)
{
  ...
  while (1) {
    ...
    if (p->type == MPC_TYPE_OR && p->data.or
      ...
      p->data.or.xs =
        realloc(p->data.or.xs, sizeof(mpc_parser_t*) * (n + m - 1));
      memmove(
        p->data.or.xs + m, p->data.or.xs + 1, (n - 1) * sizeof(mpc_parser_t*));
    ...
    if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_fold_ast &&
      ...
      p->data.and.dxs =
        realloc(p->data.and.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
      memmove(
        p->data.and.xs + n - 1, t->data.and.xs, m * sizeof(mpc_parser_t*));
      for (i = 0; i < p->data.and.n - 1; i++) {
        p->data.and.dxs[i] = (mpc_dtor_t)mpc_ast_delete;
      }
      free(t->data.and.xs);
